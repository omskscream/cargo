...
fn summary_for_patch(
    ...
    if let Some(locked) = locked {
        ...
        let orig_matches =
            ready!(source.query_vec(&original_patch.dep, QueryKind::Exact)).unwrap_or_else(|e| {
                tracing::warn!(
    ...
    let name_only_dep = Dependency::new_override(original_patch.dep.package_name(), original_patch.dep.source_id());
