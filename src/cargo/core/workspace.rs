...
use crate::util::context::{Definition, FeatureUnification, Value};
            ...
            patch.insert(
                ...
                deps.iter()
                    .map(|(name, dependency_cv)| {
                        let patch_loc = match &dependency_cv.definition {
                            Definition::Path(p) => PatchLocation::Config(Some(p.clone())),
                            Definition::Cli(_) => PatchLocation::Cli,
                            _ => PatchLocation::Config(None),
                        };
